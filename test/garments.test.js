...
describe('As part of the sql refresh workshop', () => {
    ...
    it('you should be able to find all the Winter Male garments', async() => {
        ...
        const result = await db.one(`select count(*) from garment where gender='Male' and season='Winter'`)
            // no changes below this line in this function
    ...
    it('you should be able to add 2 Male & 3 Female garments', async() => {
        ...
        await db.none(`insert into garment(description, img, season, gender, price) values ('Red hooded jacket', 'hoodie-128x128-455122.png', 'Winter', 'Male', '299.99')`);
    ...
    it('you should be group garments by gender and count them', async() => {
        ...
        const garmentsGrouped = await db.one(`select count(*),gender from garment where gender='Male' group by gender`);
        const garmentsGrouped1 = await db.one(`select count(*),gender from garment where gender='Female' group by gender`)
        const garmentsGrouped2 = await db.one(`select count(*),gender from garment where gender='Unisex' group by gender`)
        ...
        assert.deepStrictEqual(expectedResult, [garmentsGrouped, garmentsGrouped1, garmentsGrouped2])
    });
